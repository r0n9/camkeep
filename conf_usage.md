...
ONVIF 摄像头可使用自动事件源，Event/PullPoint 健康时由设备事件触发录像，并短时使用本地帧差跟踪事件持续状态；异常时回退本地帧差：
...
* 普通模式下残留的 `*_unknown.mp4` 或 `*_unknown.ts` 不参与多片段合并；每日任务会先尝试单文件修复/MP4 封装，成功后输出类似 `cam1_20260512_090000_090500_repaired.mp4` 的文件，并删除原 `_unknown` 文件。
...
* `motion_detect: true` 且 `motion_event_source` 为 `auto`，ONVIF 触发事件录像后短时使用帧差跟踪来延长/结束事件窗口。
...
* `auto`：ONVIF Event 通道健康时优先由 PullPoint 触发事件录像，并在触发后短时启动本地帧差跟踪，用于延长/结束当前事件窗口；当 ONVIF Event 不可用、PullPoint 不支持、订阅/PullMessages 失败或最近 Pull 成功超时时，自动回退本地帧差。

`auto` 判断的是 ONVIF Event 通道健康，不要求已经收到过 motion 触发。安静场景长时间没有 motion 事件不会导致回退；但 PullPoint 监听异常会回退。ONVIF 健康时不会常驻帧差，只有收到 ONVIF motion 后才会短时跟踪，以避免设备只上报稀疏事件时过早结束录像。
